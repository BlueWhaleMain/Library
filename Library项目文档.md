...
**该文档仅适用于：Library-2.5.2.2-1.16.5**
...
总体说明： 肥胖度用于参与生命参数计算，过高或过低都会造成不利影响，玩家通过饮食和运动来控制它； 疲劳度用于评估玩家的工作状态，过高会影响正常活动甚至死亡，即使挂机，仍然会持续增长，
玩家通过睡眠恢复它，亦可获得夜视效果来抑制其增长，失明时会缓慢恢复； 口渴度顾名思义，过高会影响正常工作甚至死亡， 过低会导致水中毒甚至死亡，玩家通过饮食来减少或者增加它，喝水是最有效的；
消耗程度用于表示玩家消耗的物质，即使挂机也会增长，达到最大值随即触发消耗事件，重设自身， 是一个过程量，不必特别关心； 消耗程度计算缓存用于缓存玩家的运动消耗，达到最大值即使消耗程度自增1并重设自身；
攻击亢奋程度用于计算攻击计算参数，玩家剧烈的战斗行为会或多或少的增加它，而玩家挨打， 会急剧降低，会随着时间逐渐归零，不好的状态会加速归零的过程； 防御专注程度用于计算防御计算参数，玩家挨打或者格挡（非盾牌）会使其增加，而玩家反击，
会急剧降低，会随着时间逐渐归零，不好的状态会加速归零的过程； 攻击附加参数，用于计算攻击计算参数，好的状态会使其增长，会随着时间逐渐归零， 不好的状态会加速归零的过程；
防御附加参数，用于计算防御计算参数，好的状态会使其增长，会随着时间逐渐归零， 不好的状态会加速归零的过程； 攻击计算参数，用于评估玩家的战斗状态，过低导致玩家虚弱，丧失战斗力，
...
(已经作为定值参数作为其他用途，切勿用于储存信息)； 触发器用于监视玩家的操作，并执行相应的逻辑，其中检查版本触发器可以使用trigger指令触发； 触发事件用于将多个触发器组合成新的触发逻辑，用于调用各个模块的方法；可以重复触发；
...
当数目低于默认最大值（32）且低于2*boss数目*难度的值时，生成同类 难度越高生成速度越快，生成的僵尸无法成为Boss，并且会随着时间推移死亡 死亡50%几率掉落体检器
...
合成御币： 空 空 木棍 空 木棍 纸（或布） -祭坛，充能5，冷却5> 空御币 木棍 空 纸（或布）

合成御札： 纸（Bill）放入祭坛中央，充能1，冷却1

合成小玉： 下界石英（SmallJade）放入祭坛中央，充能1，冷却1

合成阴阳玉： 空 空 空 空 小玉 御札 -祭坛，充能2，冷却2> 空阴阳玉 空 空 空
...
并且使玩家升起18个方块，弹幕与狙模式交替进行（会有不同的音效）， LastBomb会使弹幕分裂攻击，Bomb结束后缓慢下降；八云紫的支援阴阳玉收集32范围内的战利品，
...
合成御币-直线模式-札弹： 空 空 空 空 御币 御札 -祭坛，充能2，冷却2> 御币-直线模式-札弹 空 空 空
...
合成红石箭： 空 空 石英 空 木棍 空 -扩展工作台> 红石箭 红石 空 空
...
合成磁铁： 空 空 空 青金石 magnet铁锭 红石 -祭坛，充能4，冷却4> 物品磁铁 空 下界石英 空
...
合成节制器： 铁粒 红石 铁粒 铁粒 磁铁 铁粒 祭坛 充能9s 冷却9s 合成节制器X1 铁粒 石英 铁粒
...
白色绳子： 空 线 空 线 棍 线 扩展工作台 白色绳子 空 线 空

针： 铁粒重命名为Needle，丢在铁砧上

白色口袋： 空 白色绳子 空 白色布 针 白色布 扩展工作台 白色口袋 空 白色布 空
...
合成环： 空 末影珍珠 空 铁锭 工作台 铁锭 充能台，升压5s，充能10s，冷却10s 放电台10s 空 铁锭 空

合成环右键在附近摆放工作台，离开自动回收，有效距离为6

工厂环： 空 空 空 空 合成环 钻石 充能台，升压1s，充能20s，冷却20s 放电台20s 空 空 空
...
煤炭块：64个煤炭，丢入炼药锅，活塞压顶

青金石块：64个青金石，丢入炼药锅，活塞压顶

红石块：64个红石，丢入炼药锅，活塞推动铁块压顶

萤石：64个萤石粉，丢入炼药锅，活塞推动红石块压顶

钻石块：64个钻石，丢入炼药锅，活塞推动红石块压顶

绿宝石块：64个绿宝石，丢入炼药锅，活塞推动红石块压顶

骨块：64个骨粉，丢入炼药锅，活塞推动青金石块压顶
...
100≤299： 给予状态效果：2s挖掘疲劳I

200≤299： 给予状态效果：2s虚弱I
...
怕水的物品： 火药 发酵蜘蛛眼 烟火之星 烟花火箭 糖 面包 蛋糕 曲奇 干海带 烤土豆 南瓜派 牛排 熟鸡肉 熟鳕鱼 熟羊肉 熟猪排 熟兔肉 熟鲑鱼 白色染料 淡灰色染料 灰色染料 黑色染料 棕色染料 红色染料 橘黄色染料 黄色染料
黄绿色染料 绿色染料 青色染料 蓝色染料 紫色染料 品红色染料 粉色染料 烈焰弹 雪球 雪 雪块 TNT 火把
